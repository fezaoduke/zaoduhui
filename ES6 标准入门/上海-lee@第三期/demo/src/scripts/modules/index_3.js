...
const listData = filterSelectData(data, 1)
//console.log(data)

const _renderLi = function (listData) {
    return listData.map(function (li) {
        return ('\n  <li class=\"select__li\" data-id=\"'
          + (li.id) + '\">' + (li.text) + '</li>\n');
    }).join('');
};

const _renderHasTitleLi = function (listData) {
    return listData.map(function (li) {
        if ( ! li.title){
            return ('<li class=\"select__li\" data-id=\"' + (li.id) + '\">'
             + (li.text) + '</li>')
        }
...
const _renderSelect = function (data) {
